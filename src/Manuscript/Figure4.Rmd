---
title: "Figure 4 Immunoglobulin plots"
...
DEGs_df <- DATA %>%
  ...
  mutate(groups = factor(.$groups, levels = rev(sort(unique(.$groups)))))
...
violin_mean_plot <- function(DEGs_df){
  ...
  DEGs_df %>%
    ...
    #geom_boxplot(fill = "transparent", notch = T, outlier.size = .5, outlier.colour = "red") +
    geom_jitter(width = 0.3, height = 0.2, alpha = 0.2, size=.1) +
...
ggsave("./Figures/04/Fig_L1_L2_viol.png", A, width = 4, height = 4) #, dpi = 300
...
#########################
# dev.new(width=6.7, height=3, noRStudioGD = TRUE) 
B <- plot_grid(plots_rep[[4]], plots_rep[[5]], ncol = 1)
(p <- plot_grid(A, B, rel_widths = c(1, 1), labels = c("a", "b")))
ggsave("./Figures/04/Figure_4.png", p, width = 6.7, height = 3, bg = "white") #, dpi = 300
```
