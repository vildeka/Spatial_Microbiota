...
geneid <- DEGs_table %>% filter(gene %in% geneid) %>% filter(grepl("^1$|Basal", .$layers)) %>% distinct(gene, layers) %>% deframe()
(Immuno <- morf_DEGs_plot(genes = geneid, clus="^1$|8"))
```
...
(plot <- plot_grid( FIG2, C,  ncol = 2, rel_heights = c(1), rel_widths = c(1, .5)) )
