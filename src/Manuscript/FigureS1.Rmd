...
---
## 16S rRNA gene-based proportions of vaginal taxa
...
taxa_plot.fun <- function(type, area=F, txt_size=12){
  ...
  labs <- d %>% map(., ~levels(.x$taxa)) 

  # shows italics for bacterial names
  library(ggtext)
  labs <- case_when(
    labs$L1 %in% n ~ paste0("<i>", labs$L1, "</i>"),
    TRUE ~ labs$L1
  )
  ...
  ggplot() + 
    ...
    aesthetics = c("color", "fill") ) + theme_classic(base_size = txt_size) +
...
#####################################
# normalize function:
# NB! Im not sure if this is the best way to normalize the data
normalize.fun <- function(df){round( t(log2( t(df) / colSums(df) * 1000 + 1 )),4)}
...
get_NMDS.fun <- function(df, gr="L"){
  ...
  example_NMDS=metaMDS(df, # Our normalized counts samples (row) by species (col) matrix
...
```{r FigureS1, fig.width=9, fig.height=5, dpi=1000}
#############################
# COMBINE ALL FIGURE PANELS #
#############################
# dev.new(width=9, height=5, noRStudioGD = TRUE)
FigureS1 <- plot_grid( A, B$L, ncol=2, rel_widths = c(1,.9)) 
# ggsave("./Figures/S1/Figure_S1.pdf", FigureS1, width = 9, height = 5, bg = "white", dpi = 1000)
